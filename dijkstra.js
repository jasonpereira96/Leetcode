...
var networkDelayTime = function (times, N, K) {
    ...
    var adjList = new Array(N + 1);
    for (var i = 1; i <= N; i++) {
        adjList[i] = [];
    }

    for (var time of times) {
        var [from, to, weight] = time;
        adjList[from].push({
            node: to,
            weight
        });
    ...
    while (visitedCount !== N) {
        ...
        var neighbours = adjList[nodeId];

        for (var neighbour of neighbours) {
            var { weight } = neighbour;
            var neighbourNode = neighbour.node;

            if (distance[nodeId] + weight < distance[neighbourNode]) {
                distance[neighbourNode] = distance[nodeId] + weight;
            }
        }
    ...
    for (var index = 1; index <= N; index++) {
        if (distance[index] > maxTime) {
            maxTime = distance[index];
        }
    }
    return maxTime;
